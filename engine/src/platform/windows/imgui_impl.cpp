/*
 * Created by Kami-Kaze on 4/17/2021.
 */
...
static Engine::GlfwWindow*          n_Window = nullptr;
...
{
    if (g_PrevUserCallbackMousebutton != NULL && window == n_Window->getGlfwWindow())
...
{
    if (g_PrevUserCallbackScroll != NULL && window == n_Window->getGlfwWindow())
...
{
    if (g_PrevUserCallbackKey != NULL && window == n_Window->getGlfwWindow())
...
{
    if (g_PrevUserCallbackChar != NULL && window == n_Window->getGlfwWindow())
...
static bool ImGui_ImplGlfw_Init(Engine::GlfwWindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    n_Window = window;
    g_Time = 0.0;
    ...
    io.ClipboardUserData = n_Window->getGlfwWindow();
    ...
    if (install_callbacks)
    {
        ...
        g_PrevUserCallbackMousebutton = glfwSetMouseButtonCallback(n_Window->getGlfwWindow(), ImGui_ImplGlfw_MouseButtonCallback);
        g_PrevUserCallbackScroll = glfwSetScrollCallback(n_Window->getGlfwWindow(), ImGui_ImplGlfw_ScrollCallback);
        g_PrevUserCallbackKey = glfwSetKeyCallback(n_Window->getGlfwWindow(), ImGui_ImplGlfw_KeyCallback);
        g_PrevUserCallbackChar = glfwSetCharCallback(n_Window->getGlfwWindow(), ImGui_ImplGlfw_CharCallback);
    ...
    main_viewport->PlatformHandle = n_Window->getNativeWindow();
#ifdef _WIN32
    main_viewport->PlatformHandleRaw = glfwGetWin32Window(n_Window->getGlfwWindow());
#endif
...
bool ImGui_ImplGlfw_InitForOpenGL(Engine::GlfwWindow* window, bool install_callbacks)
{
...
void ImGui_ImplGlfw_Shutdown()
{
    ...
    {
        glfwSetMouseButtonCallback(n_Window->getGlfwWindow(), g_PrevUserCallbackMousebutton);
        glfwSetScrollCallback(n_Window->getGlfwWindow(), g_PrevUserCallbackScroll);
        glfwSetKeyCallback(n_Window->getGlfwWindow(), g_PrevUserCallbackKey);
        glfwSetCharCallback(n_Window->getGlfwWindow(), g_PrevUserCallbackChar);
...
static void ImGui_ImplGlfw_UpdateMousePosAndButtons()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++)
    {
        ...
        io.MouseDown[i] = g_MouseJustPressed[i] || glfwGetMouseButton(n_Window->getGlfwWindow(), i) != 0;
...
#else
        const bool focused = n_Window->isFocused();
#endif
        if (focused)
        {
...
{
    ImGuiIO& io = ImGui::GetIO();
    if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) || glfwGetInputMode(n_Window->getGlfwWindow(), GLFW_CURSOR) == GLFW_CURSOR_DISABLED)
        return;
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    glfwGetWindowSize(n_Window->getGlfwWindow(), &w, &h);
    glfwGetFramebufferSize(n_Window->getGlfwWindow(), &display_w, &display_h);
...
#endif
    GLFWwindow* share_window = (g_ClientApi == GlfwClientApi_OpenGL) ? n_Window->getGlfwWindow() : NULL;
...
static void ImGui_ImplGlfw_InitPlatformInterface()
{
    ...
    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    ImGuiViewportDataGlfw* data = IM_NEW(ImGuiViewportDataGlfw)();
    data->Window = n_Window->getGlfwWindow();
